...
open class JLEcDocumentBrowserView: JLEcBaseView, UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if #available(iOS 13.0, *) {
            let icon = isDir.boolValue ? UIImage(systemName: "folder") : UIImage(systemName: "doc")
            cell.imageView?.image = icon
        } else {
            cell.imageView?.image = isDir.boolValue ? UIImage(named: "folder") : UIImage(named: "doc")
        }
